...
async function analyzeFile() {
  try {
    ...
    const ventasSheet = workbook.getWorksheet('Ventas combinadas');
    if (ventasSheet) {
      const ventasData = worksheetToJson(ventasSheet);
    ...
    const kenpSheet = workbook.getWorksheet('KENP leídas');
    if (kenpSheet) {
      const kenpData = worksheetToJson(kenpSheet);
    ...
    const pedidosSheet = workbook.getWorksheet('Pedidos completados de eBooks');
    if (pedidosSheet) {
      const pedidosData = worksheetToJson(pedidosSheet);
